...
from modules.net_models import mean_pooling, AttentionPooling
...
class AutoGraderModel(Wav2Vec2PreTrainedModel):
    ...
    def __init__(self, model_args, class_weight=None, config=None, text_config=None, pretrained=False):
        ...
        self.speech_attn_pool = AttentionPooling(in_dim=self.config.hidden_size)
        # prediction head
    ...
    def forward(self,
        ...
        #text_hidden_states = text_hidden_states[:,0,:] # B, H 
        #text_hidden_states = text_hidden_states.unsqueeze(1).expand(-1, hidden_states.size(1), -1)
        
        # prompt (text)
        #prompt_hidden_states = prompt_outputs["last_hidden_state"] # B, T, H       
        #prompt_hidden_states = prompt_hidden_states[:,0,:] # B, H 
        #prompt_hidden_states = prompt_hidden_states.unsqueeze(1).expand(-1, hidden_states.size(1), -1)
        
        # response (audio)
        padding_mask = self._get_feature_vector_attention_mask(hidden_states.shape[1], attention_mask)
        #hidden_states = torch.cat([hidden_states, text_hidden_states], dim=-1)
        hidden_states = self.speech_attn_pool(x=hidden_states, attn=hidden_states, mask=padding_mask)
...
class AutoMAGraderModel(Wav2Vec2PreTrainedModel):
    ...
    def __init__(self, model_args, class_weight=None, config=None, text_config=None, pretrained=False):
        ...
        self.contetn_pooling = AttentionPooling(in_dim=self.config.hidden_size)
    ...
    def forward(self,
        ...
        content_vector = self.content_pooling(x=content_vector, attn=content_vector, mask=attention_mask)

        # delivery
        delivery_vector = self.delivery_encoder(x=delivery, mask=delivery_mask)
        delivery_vector = self.delivery_pooling(x=delivery_vector, attn=delivery_vector, mask=delivery_mask)

        # language use
        language_use_vector = self.language_use_encoder(x=language_use, mask=language_use_mask)
        language_use_vector = self.language_use_pooling(x=language_use_vector, attn=language_use_vector, mask=language_use_mask)
